! function() {
    ...
    function k() {
        ...
        n.length || (s += '<div class="goal"><div class="gmid"><div class="gname" style="color:var(--ink-soft)">—</div></div></div>'), n.forEach(function(t) {
            var a = t.price > 0 ? Math.min(100, Math.round(e / t.price * 100)) : 0,
                n = e >= t.price;
            s += '<div class="goal' + (n ? " reached" : "") + '"><div class="gpic">' + t.icon + '</div><div class="gmid"><div class="gname">' + t.name + '</div><div class="gprice">R' + e + " / R" + t.price + '</div><div class="bar"><div class="fill" data-pct="' + a + '">' + (n ? "✓" : "") + "</div></div></div></div>"
    ...
    function W() {
        ...
        var c = Core.getState().settings,
            screenTimes = c.screenTimes || [{start: "18:00", end: "19:00"}],
            x = s * 60 + e;
        ...
        if (k && screenTimes.length > 0) {
            var isScreenTimeNow = !1;
            screenTimes.forEach(function(t) {
                var i = t.start.split(":"), l = t.end.split(":"), r = parseInt(i[0]) * 60 + parseInt(i[1]), m = parseInt(l[0]) * 60 + parseInt(l[1]);
                if (r < m && x >= r && x < m) isScreenTimeNow = !0;
                else if (r >= m && (x >= r || x < m)) isScreenTimeNow = !0
            }), k.style.display = isScreenTimeNow ? "" : "none"
        } else if (k) {
    ...
    function q() {
        ...
        "today" === l ? n.innerHTML = function() {
            ...
            var s = Core.getState().settings.screenTimes || [], c = !1, r = s.map(function(t) { return t.start + "–" + t.end }).join(", ");
            if (s.length > 0) {
                var i = new Date().toTimeString().substr(0, 5);
                c = s.some(function(t) { return i >= t.start && i <= t.end })
            }
            e += '<div class="arow" data-child="' + t.id + '"><div class="ic" style="background:' + t.colorLite + '">' + t.av + '</div><div class="g"><div class="t">' + t.name + '</div><div class="s"><span class="tag" style="background:' + t.colorLite + ";color:" + t.color + '">colour</span><span class="tag gold">R' + t.balance + '</span>' + (r ? '<span class="tag">' + (c ? '📱' : '🚫') + ' ' + r + '</span>' : '') + '</div></div><span class="chev">›</span></div>'
    ...
    function Y(t) {
        ...
        e += '<label class="f">What is it?</label><input class="tf" data-field="name" value="' + et(a.name) + '">', e += '<label class="f">Price (R)</label><input class="tf" data-field="price" value="' + (a.price || "") + '" inputmode="numeric">', e += '<label class="f">Show now?</label><div class="seg" data-seg="status"><button class="' + ("active" === a.status ? "on" : "") + '" data-val="active">Active</button><button class="' + ("wishlist" === a.status ? "on" : "") + '" data-val="wishlist">Wishlist</button></div>', e += '<div style="height:12px"></div><button class="btn pri" data-act="save-goal">Save goal</button>';
    ...
    Q = document.getElementById("pad"), [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach(function(t) {
                                ...
                                switch (e) {
                                    ...
                                    case "save-goal":
                                        ! function() {
                                            var t = z(),
                                                a = J;
                                            a.price = parseInt(a.price, 10) || 0, a._new ? (delete a._new, Core.addGoal(t.id, a)) : Core.updateGoal(a.id, {
                                                name: a.name,
                                                price: a.price,
                                                icon: a.icon,
                                                status: a.status
        ...
        if (a) {
            ...
            if (e && e.startsWith("screen")) {
                ...
                if (e.startsWith("screenStart_") || e.startsWith("screenEnd_")) {
                    var s = e.split("_")[1];
                    if (!n.screenTimes) n.screenTimes = [];
                    if (!n.screenTimes[s]) n.screenTimes[s] = {};
                    if (e.startsWith("screenStart_")) n.screenTimes[s].start = a.value;
                    else n.screenTimes[s].end = a.value;
                } else if (e.startsWith("screenTime")) {
                    n[e] = a.value
                }
                Core.save(), document.getElementById("admin").classList.contains("show") && q(), W()
            } else J && (J[e] = a.value)
        }
    });
    var it = !1;
